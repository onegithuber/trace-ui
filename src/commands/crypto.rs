...
) -> Result<Option<CryptoScanResult>, String> {
    let mut sessions = state.sessions.write().map_err(|e| e.to_string())?;
    let session = sessions.get_mut(&session_id)
        .ok_or_else(|| format!("Session {} not found", session_id))?;
    if session.crypto_cache.is_some() {
        return Ok(session.crypto_cache.clone());
    }
    if let Some(cached) = crate::cache::load_crypto_cache(&session.file_path, &session.mmap) {
        session.crypto_cache = Some(cached.clone());
...
pub async fn scan_crypto(
    ...
    // 检查内存缓存
    {
        ...
        let session = sessions.get(&session_id)
            .ok_or_else(|| format!("Session {} not found", session_id))?;
        if let Some(cached) = &session.crypto_cache {
    ...
    // 写入内存缓存 + 磁盘缓存
    {
        let mut sessions = state.sessions.write().map_err(|e| e.to_string())?;
        if let Some(session) = sessions.get_mut(&session_id) {
            crate::cache::save_crypto_cache(&session.file_path, &session.mmap, &result);
            session.crypto_cache = Some(result.clone());
        }
    }
