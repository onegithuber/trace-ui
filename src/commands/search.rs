...
pub async fn search_trace(
...
) -> Result<SearchResultLite, String> {
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        ...
        if let Some(chunks) = chunks {
            // 并行搜索各分块
            use rayon::prelude::*;
            let chunk_results: Vec<(Vec<u32>, u32)> = chunks.par_iter()
...
pub async fn get_search_matches(
    ...
    if request.seqs.is_empty() {
        return Ok(Vec::new());
    }

    let mode = parse_search_mode(&request.query, request.case_sensitive, request.use_regex, request.fuzzy)?;

    // Extract data from session while holding lock
    let (mmap_arc, trace_format, call_search_texts, call_annotations, li_data) = {
        let sessions = state.sessions.read().map_err(|e| e.to_string())?;
        let session = sessions.get(&session_id)
            .ok_or_else(|| format!("Session {} 不存在", session_id))?;

        let li = session.line_index_view()
            .ok_or_else(|| "Line index not available".to_string())?;

        let data: &[u8] = &session.mmap;
        // Pre-compute byte offsets for requested seqs while holding the lock
        // Filter out consumed_seqs defensively
        let consumed: std::collections::HashSet<u32> = session.consumed_seqs.iter().copied().collect();
        let offsets: Vec<(u32, usize)> = request.seqs.iter()
        ...
        for (seq, offset) in li_data {
            ...
            if let Some(parsed) = parsed {
                ...
                let call_info = call_annotations.get(&seq).map(|ann| {
                    ...
                    let display_summary = if summary.len() > 80 {
                        let mut end = 80;
                        while !summary.is_char_boundary(end) && end > 0 { end -= 1; }
                        &summary[..end]
                    } else {
                        &summary
                    };
                    let rendered_text = rendered_search_text(
                        &parsed.address, &parsed.disasm, &parsed.changes,
                        parsed.mem_rw.as_deref(), Some(display_summary),
                    );
                    let annotation_match = call_search_texts.get(&seq)
                        .map_or(false, |text| matches_mode_str(&mode, text));
                    if annotation_match && !matches_mode_str(&mode, &rendered_text) && !tooltip.is_empty() {
                        hidden_content = Some(tooltip.clone());
                    }
                    CallInfoDto {
                        func_name: ann.func_name.clone(),
    ...
    .await
    .map_err(|e| format!("get_search_matches panic: {}", e))?;
