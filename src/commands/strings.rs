...
use crate::taint::strings::{StringEncoding, StringRw};
...
pub async fn scan_strings(
    ...
    // 1. Collect READ+WRITE records and get cancellation flag
    let (mut accesses, cancelled) = {
        let sessions = state.sessions.read().map_err(|e| e.to_string())?;
        let session = sessions.get(&session_id)
            .ok_or_else(|| format!("Session {} 不存在", session_id))?;
        let mem_view = session.mem_accesses_view().ok_or("索引尚未构建完成")?;

        let mut accesses: Vec<(u64, u64, u8, u32, StringRw)> = Vec::new();
        ...
        (accesses, session.scan_strings_cancelled.clone())
    };
